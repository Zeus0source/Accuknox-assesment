...
Solutions to Problem Statement 1 of the AccuKnox AI/ML Trainee assessment.
 ...
 File Description:
 1. ps1_sol_1.py  Fetches books data (title, author, publication year) from the Open Library public API, stores it in a local SQLite DB, and displays the records. 

2.  ps1_sol_2.py  Calculates each student's average test score across subjects and visualizes the results as a bar chart. Note: no publicly available REST API serving student test score data in this format was found; a  mock dataset is used for 
visualization, as documented in the script. 

 3. ps1_sol_3.py Reads user data (users.csv) and inserts it into a local SQLite DB. 
